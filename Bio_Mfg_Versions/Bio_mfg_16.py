...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 22:55:06 2019
...
print ("Harvesting Yield For Patient : \n", yield_harvesting)

#Selecing times based on the product_mix
...
def main_call(mfg_machine, mfg_operator, product_mix,run):
    ...
    df['Yield_after_harvesting'] = yield_harvest
    ...
    Harv_numbers = np.array((Arrival_times, harv_setup_times,harv_service_times, harv_wait_time, harv_total_time, harv_Departure_times), dtype=float)
    Harv_numbers = np.transpose(Harv_numbers)
    ...
    #expected after cryo taken as 97% of the initial expected yield
    exp_yield_after_cryo = []
    for y in patients_target_bc:
        exp_yield_after_cryo.append(y*0.97)
    df['exp_yield_after_cryo'] = exp_yield_after_cryo    
    
    #actual yield after cryo
    beta = 10
    df['act_yield_after_cryo'] = df['Yield_after_harvesting']-beta* df['cryo_total_time']   
    ...
    #expected yield after cryo taken as 90% of the initial expected yield
    exp_yield_before_mfg = []
    for y_t in patients_target_bc:
        exp_yield_before_mfg.append(y_t*0.9)
    df['exp_yield_before_mfg'] = exp_yield_before_mfg 
    ...
    # drawn from triangular dist between 3.75 and 4.25 hours
    # with  mode and mean at 4 hour
    mfg_service_times = np.random.triangular(left=288, mode=312, right=336, size=NUM_MACHINES_MFG)
    ...
    #waiting before mfg
    waiting_before_mfg = tnsprt_times + mfg_sample_wait_time
    df['waiting_before_mfg'] = waiting_before_mfg
    
    #act_yield_before_mfg
    zeta = 25
    df['act_yield_before_mfg'] = df['act_yield_after_cryo'] -  zeta*df['waiting_before_mfg']
...
df_consol.to_excel(r'/Users/gauravsharma/Documents/Simulation_results.xls')